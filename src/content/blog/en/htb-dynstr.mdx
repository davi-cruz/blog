---
title: 'Walkthrough: HTB Dynstr'
description: This week's machine will be Dynstr, another medium-rated Linux box from
  Hack The Box, created by jkr.
pubDate: '2021-10-16'
tags:
- HackTheBox
- HTB Medium
- HTB Linux
- Walkthrough
translationId: htb-dynstr
draft: false
heroImage: https://i.imgur.com/89b1SdH.png
---
...
![HTB Dynstr](https://i.imgur.com/kqW8fwk.png)
...
![HTB Dynstr - Dyna DNS](https://i.imgur.com/gtdmYDI.jpg)

Besides a single-page application, this website contains especially useful information in the section _Our Services_, that will be valuable to us during this box resolution, as well as the domain `dynadns.htb`, used in the e-mail address in the footer:

![HTB Dynstr - Dyna DNS Services](https://i.imgur.com/QZvDDXJ.png)
...
![HTB Dynstr - OS Code injection payload](https://i.imgur.com/r5qrIOo.png)
...
One point seen in the page mentioned above is that we could use the parameter `--preserve` to **preserve file attributes** during the copy. Appending this parameter to execution could be achieved thanks to a `bash` feature called **filename expansion**, popularly known as **_[globbing](https://tldp.org/LDP/abs/html/globbingref.html)_** which **expands** the names of the files during command execution. This behavior can be seen in the example below with `echo`:
