...
description: Often I help customers on deploying CEF/Syslog forwarders in their environments
  to gather data from Network Appliances and/or other servers and services into Lo...
pubDate: '2021-03-29'
tags:
- Microsoft Sentinel
- Syslog
...
<Alert kind="tip" title="Note">
  If you don't have python 2.7 in your system you might need to replace `python` by `python3` prior
  to executing the line below.
</Alert>
...
<Alert kind="tip" title="Note">
  The location you can find this default configuration vary depending on your linux distribution.
  For Ubuntu there's a file named `/etc/rsyslog.d/50-default.conf` while for RHEL based
  distributions (RHEL, CentOS, Fedora) it is in the main `/etc/rsyslog.conf`.
</Alert>
...
To prevent this behavior, as Ofer Shezaf shared in one of the [Security Community Webinars](https://aka.ms/securitywebinars) (_Log Forwarder deep dive \| Filtering CEF and Syslog events_), we'll rename the file `security-config-omsagent.conf` to `60-cef.conf` so it can be processed **before** `95-omsagent.conf` file.
...
<Alert kind="tip" title="Note">
  You may find some variations of the stop instruction as a tilde (`~`). This came from the legacy
  syslogd syntax
</Alert>
...
<Alert kind="tip" title="Note">
  In syslogd syntax a single at sign (`@`) means UDP while double at sign (`@@`) means TCP.
</Alert>

<Alert kind="warning" title="Warning">
  In the previously mentioned Deep Dive CEF webinar, Ofer also recommend you change the omsagent
  configuration to accept messages using TCP. If you follow his recommendation, you also need to
  adjust any forwarding instruction to double at signs in your `95-omsagent.conf` file, otherwise
  the messages won't be sent to Log Analytics.
</Alert>
