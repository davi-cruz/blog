...
\:information\_source: **Info**: Write-ups for Hack The Box machines are posted as soon as they’re retired.
\[info]
...
As expected, once reviewing the request history, we can see the account activation details from Mattermost, sent to **[8024065@delivery.htb](mailto:8024065@delivery.htb)**.
...
* osTicket Admin Credentials were **maildeliverer\:Youve\_G0t\_Mail!**.
...
Besides several attempts using osTicket API, gave some steps back and thinking simple, asked me: Is **maildeliverer** a user at this machine? On I tried to SSH using its account I was able to not only connect to this machine (\:man\_facepalming:) but also get the flag in `user.txt`, available at his root directory.
...
As I couldn't read any of the two promising files for path hijack, started looking for credentials to get access to MySQL and try to dump some credentials. In osTicket install folder, have found `/var/www/osticket/upload/include/ost-config.php` which contained the creds for **ost\_user**:
...
Using the credentials found (**mmuser\:Crack\_The\_MM\_Admin\_PW**) connected to **mattermost** database and, at users table, found some password hashes, being **root** the only with **system\_admin** permissions.
