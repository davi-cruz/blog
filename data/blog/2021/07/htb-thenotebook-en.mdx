...
The fact is: We cannot modify the issued token without signing it again with the key available in the server, but, once the token has a field pointing to the path to the key to which it was signed, we could get a chance to recreate this token, signing it with our own RSA key and **making it available to the backend** during our credential validation, but we can only be sure testing \:stuck\_out\_tongue\_winking\_eye:.
