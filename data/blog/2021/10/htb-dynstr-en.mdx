...
Checking its content, noticed one interesting entry in **line 42**, where the script, running as `root`, copies the file `.version` and all other existing files in the current directory (`*`) to $BINDMGR\_DIR. Also, it requires a `.version` file in the current directory to be compared with the existing configuration in the server.
