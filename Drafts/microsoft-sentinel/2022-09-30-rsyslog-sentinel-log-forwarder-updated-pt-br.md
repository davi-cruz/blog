---
layout: single
title: "Microsoft Sentinel: Configuração do Log Forwarder [Atualizado]"
namespace: rsyslog-sentinel-log-forwarder-updated
...
<div class="mermaid">
graph TD
    subgraph A[ /etc/rsyslog.d/*.*]
        B[ /etc/rsyslog.d/50-default.conf]
        C[ /etc/rsyslog.d/95-omsagent.conf]
        D[ /etc/rsyslog.d/security-config-omsagent.conf]
        B --> C --> D
    end
    E[ /etc/rsyslog.conf] -->|imports| A
</div>
...
graph TD
    subgraph A[ /etc/rsyslog.d/*.*]
        B[ /etc/rsyslog.d/50-default.conf]
        C[ /etc/rsyslog.d/60-cef.conf]
        D[ /etc/rsyslog.d/95-omsagent.conf]
        B --> C --> D
    end
    E[ /etc/rsyslog.conf] -->|imports| A
    style C fill:#228B22;
...
graph TD
    subgraph A[ /etc/rsyslog.d/*.*]
        B[ /etc/rsyslog.d/50-default.conf]
        C[ /etc/rsyslog.d/59-filter-out.conf]
        D[ /etc/rsyslog.d/60-cef.conf]
        E[ /etc/rsyslog.d/95-omsagent.conf]
        B --> C --> D --> E
    end
    F[ /etc/rsyslog.conf] -->|imports| A
    style C fill:#228B22;
</div>
